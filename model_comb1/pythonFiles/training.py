...
from utils.gwo import GreyWolfOptimizer
...
# ─────────────────────────────────────────────────────────────
# PATHS  — SavedModels/ at project root
# ─────────────────────────────────────────────────────────────
...
PATH_BOX   = mp("CheckPoint1.h5")
PATH_CLS   = mp("classNbox.h5")
PATH_SEG   = mp("segmentation.h5")
PATH_COLON = mp("ColonNet.h5")
PATH_GWO   = mp("gwo_best_params.json")
...
# Objective optimises BOTH branches (box + classification weighted
# equally) so the found LR is valid for all stages.
# Result cached to PATH_GWO — skipped on every subsequent run.
# ─────────────────────────────────────────────────────────────

if not os.path.exists(PATH_GWO):
    print("Loading dataset for GWO search …")
    _gwo_imgs, _gwo_boxes, _gwo_labels = load_data(with_neg=True, aug=False)
    N   = min(512, len(_gwo_imgs))
    idx = np.random.permutation(len(_gwo_imgs))[:N]
    _gwo_imgs   = _gwo_imgs[idx]
    _gwo_boxes  = _gwo_boxes[idx]
    _gwo_labels = _gwo_labels[idx]
    print(f"GWO subset shape: {_gwo_imgs.shape}")


def gwo_objective(pos):
    try:
        lr          = 10.0 ** float(pos[0])
    ...
    Xtr, Xval, btr, bval, ltr, lval = train_test_split(
        _gwo_imgs, _gwo_boxes, _gwo_labels, test_size=0.2, random_state=42
    )
    ...
    model.compile(
        ...
        loss=LOSSES,
        loss_weights={"c_final": 1.0, "b_final": 1.0},
    )
    history = model.fit(
...
if os.path.exists(PATH_GWO):
    with open(PATH_GWO) as f:
        _p = json.load(f)
    best_lr      = _p["lr"]
    best_dropout = _p["dropout"]
    best_scale   = _p["dense_scale"]
    print(f"GWO params loaded → LR={best_lr:.4e}  "
    ...
    with open(PATH_GWO, "w") as f:
        json.dump({"lr": best_lr, "dropout": best_dropout,
                   "dense_scale": best_scale}, f, indent=2)
    print(f"GWO done → LR={best_lr:.4e}  "
          f"dropout={best_dropout:.3f}  scale={best_scale:.3f}")
    print(f"Saved → {PATH_GWO}")

tf.keras.backend.clear_session()


# =====================================================
# STAGE 1 — BOUNDING BOX  (bleeding images only)
# =====================================================
print("\n" + "=" * 55)
...
if os.path.exists(PATH_SEG):
    ...
    print("  No existing weights — building fresh U-Net.")
